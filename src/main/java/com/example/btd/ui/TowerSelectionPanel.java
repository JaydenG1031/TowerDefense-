        ...
        button.setOnAction(e -> {
            if (gameManager.getMoney() >= cost) {
                ...
                getScene().setOnMouseClicked(event -> {
                    // Get the current game canvas size for proper tower scaling
                    double gameAreaWidth = getScene().getWidth() - PANEL_WIDTH;
                    
                    if (event.getX() < gameAreaWidth) {
                        gameManager.placeTower(event.getX(), event.getY(), towerType);
                        button.setDisable(gameManager.getMoney() < cost);
