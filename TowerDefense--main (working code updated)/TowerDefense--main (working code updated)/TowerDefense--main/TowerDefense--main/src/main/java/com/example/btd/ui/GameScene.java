    ...
    public GameScene(double gameAreaWidth, double gameAreaHeight, double panelWidth, String playerName) {
        ...
        gameCanvas.setOnMouseClicked(event -> {
            if (event.getButton() == javafx.scene.input.MouseButton.SECONDARY) {
                // Right click - cancel tower placement if active, otherwise delete tower
                if (gameManager.isPreviewActive()) {
                    // Cancel tower preview
                    gameManager.cancelTowerPreview();
                } else if (gameManager.deleteTower(event.getX(), event.getY())) {
                    // Show refund message
                    showRefundMessage(event.getX(), event.getY());
                }
            } else if (event.getButton() == javafx.scene.input.MouseButton.PRIMARY) {
                // Left click - handle tower selection or placement
                if (gameManager.isPreviewActive()) {
                    // Complete tower placement
                    gameManager.placeTower(event.getX(), event.getY(), null);
                    gameManager.cancelTowerPreview();
                } else {
        ...
        // Add mouse move handler for preview tower tracking
        gameCanvas.setOnMouseMoved(event -> {
            if (gameManager.isPreviewActive()) {
                gameManager.updateTowerPreview(event.getX(), event.getY());
            }
        });
        
        // Add escape key handler to cancel preview
